...
function onPeriodChange(event: Event) {
    ...
    router.reload({
        data: { period: value },
        only: ['period', 'stats', 'statusBreakdown', 'recentInvoices', 'overdueInvoices', 'topCustomers', 'monthlyTrend', 'estimateStats'],
    });
}
...
function trendBarHeight(value: number): string {
    const pct = trendMax.value > 0 ? Math.max(2, (value / trendMax.value) * 100) : 2;
    return `${pct}%`;
}

const totalStatusCount = computed(() => props.statusBreakdown.reduce((sum, s) => sum + s.count, 0));
</script>
    ...
    <AppLayout title="Dashboard">
        <div class="px-4 sm:px-0">
            ...
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <select
                    :value="period"
                    class="rounded-md border-gray-300 shadow-sm text-sm focus:ring-brand-500 focus:border-brand-500"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div class="mt-1 text-sm text-gray-500">{{ stats.invoice_count }} {{ stats.invoice_count === 1 ? 'invoice' : 'invoices' }}</div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div class="mt-1 text-sm" :class="stats.overdue_count > 0 ? 'text-red-600 font-medium' : 'text-gray-500'">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div class="mt-2 text-2xl font-bold text-gray-900">{{ customerCount }}</div>
                    <div class="mt-1 text-sm text-gray-500">
                        {{ estimateStats.count }} {{ estimateStats.count === 1 ? 'estimate' : 'estimates' }}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div class="space-y-3">
                        <template v-for="item in statusBreakdown" :key="item.status">
                        ...
                        <p v-if="totalStatusCount === 0" class="text-sm text-gray-400 text-center py-4">
                ...
                <div class="bg-white rounded-lg shadow p-5 lg:col-span-2">
                    ...
                    <div class="flex items-end justify-between gap-2 h-40">
                        <div v-for="month in monthlyTrend" :key="month.label" class="flex-1 flex flex-col items-center gap-1">
                            <div class="w-full flex flex-col items-center justify-end h-28 gap-0.5">
                                <div
                                    ...
                                    :style="{ height: trendBarHeight(month.invoiced) }"
                                    :title="'Invoiced: ' + fmt(month.invoiced)"
                                />
                                <div
                                    v-if="month.collected > 0"
                                    class="w-full max-w-[2rem] rounded-t bg-green-400 transition-all"
                                    :style="{ height: trendBarHeight(month.collected) }"
                                    :title="'Collected: ' + fmt(month.collected)"
                                />
                            </div>
                    ...
                    <div class="mt-3 flex items-center gap-4 text-xs text-gray-500">
                        <span class="flex items-center gap-1"><span class="inline-block w-3 h-3 rounded bg-brand-200" /> Invoiced</span>
                        <span class="flex items-center gap-1"><span class="inline-block w-3 h-3 rounded bg-green-400" /> Collected</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div v-if="topCustomers.length > 0" class="space-y-3">
                        <div v-for="cust in topCustomers" :key="cust.name" class="flex items-center justify-between">
                            <div>
                                <div class="text-sm font-medium text-gray-900">{{ cust.name }}</div>
                                <div class="text-xs text-gray-500">{{ cust.invoice_count }} {{ cust.invoice_count === 1 ? 'invoice' : 'invoices' }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-sm font-medium text-gray-900">{{ fmt(cust.total) }}</div>
                                <div v-if="cust.outstanding > 0" class="text-xs text-amber-600">{{ fmt(cust.outstanding) }} due</div>
                                <div v-else class="text-xs text-green-600">Fully paid</div>
                            </div>
                        </div>
                    </div>
                    <p v-else class="text-sm text-gray-400 text-center py-4">No customer data in this period</p>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    <h3 class="text-sm font-semibold text-gray-900 mb-4">
                        Overdue Invoices
                        <span v-if="overdueInvoices.length > 0" class="ml-2 inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">
                            {{ overdueInvoices.length }}
                        </span>
                    </h3>
                    <div v-if="overdueInvoices.length > 0" class="space-y-3">
                        <div v-for="inv in overdueInvoices" :key="inv.id" class="flex items-center justify-between -mx-2 px-2 py-1 rounded hover:bg-gray-50">
                            <div>
                                <div class="text-sm font-medium text-gray-900">{{ inv.invoice_number }}</div>
                                <div class="text-xs text-gray-500">{{ inv.customer_name }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-sm font-medium text-red-600">{{ formatMoney(inv.remaining_balance, inv.currency) }}</div>
                                <div class="text-xs text-red-500">Due {{ inv.due_at_human }}</div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div v-if="recentInvoices.length > 0" class="space-y-3">
                        <div v-for="inv in recentInvoices" :key="inv.id" class="flex items-center justify-between -mx-2 px-2 py-1 rounded hover:bg-gray-50">
                            <div>
                                <div class="text-sm font-medium text-gray-900">{{ inv.invoice_number }}</div>
                                <div class="text-xs text-gray-500">{{ inv.customer_name }} &middot; {{ inv.issued_at }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-sm font-medium text-gray-900">{{ formatMoney(inv.total, inv.currency) }}</div>
                                <StatusBadge :status="inv.status" />
                            </div>
                        </div>
                    </div>
                    <p v-else class="text-sm text-gray-400 text-center py-4">No invoices yet</p>
                </div>
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500">
                                    ...
                                    <template v-if="payment.payment_method">
                                        &middot; {{ payment.payment_method.replace('_', ' ') }}
                                    </template>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-sm font-medium text-green-600">+{{ formatMoney(payment.amount, payment.currency) }}</div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <a href="/invoices/create" class="flex items-center gap-3 bg-white rounded-lg shadow p-4 hover:bg-brand-50 transition-colors">
                ...
                <a href="/estimates/create" class="flex items-center gap-3 bg-white rounded-lg shadow p-4 hover:bg-green-50 transition-colors">
                ...
                <a href="/customers" class="flex items-center gap-3 bg-white rounded-lg shadow p-4 hover:bg-purple-50 transition-colors">
                ...
                <a href="/numbering-series" class="flex items-center gap-3 bg-white rounded-lg shadow p-4 hover:bg-gray-50 transition-colors">
