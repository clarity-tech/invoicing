    ...
    useForm: (initial: Record<string, string>) => {
        ...
        return new Proxy(formData, {
            get(target, prop) {
                ...
                if (prop === 'reset')
                    return () => {
                        Object.keys(initial).forEach((k) => (formData[k] = initial[k]));
                        mockReset();
                    };
...
const countries = { IN: 'India', US: 'United States', AE: 'United Arab Emirates' };
...
describe('LocationModal', () => {
    ...
    it('shows "Edit Location" title when location prop provided', () => {
        const wrapper = mount(LocationModal, {
            props: { show: true, customerId: 1, location: makeLocation(), countries },
            global: { stubs: { Teleport: true } },
        });
    ...
    it('calls form.post() on submit for new location', async () => {
        ...
        expect(mockPost).toHaveBeenCalledWith('/customers/5/locations', expect.any(Object));
    });
    ...
    it('calls form.put() on submit for existing location', async () => {
        ...
        await wrapper.find('form').trigger('submit');
        expect(mockPut).toHaveBeenCalledWith('/customers/5/locations/10', expect.any(Object));
    });
    ...
    it('shows "Add Location" on submit button for new', () => {
        ...
        expect(wrapper.find('button[type="submit"]').text()).toContain('Add Location');
    });
    ...
    it('shows "Update Location" on submit button for edit', () => {
        ...
        expect(wrapper.find('button[type="submit"]').text()).toContain('Update Location');
    });
});
