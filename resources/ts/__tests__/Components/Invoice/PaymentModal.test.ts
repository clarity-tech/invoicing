...
vi.mock('@/composables/useFormatMoney', () => ({
    formatMoney: (amount: number, currency: string) => {
        ...
        const symbols: Record<string, string> = { INR: '₹', USD: '$', AED: 'AED' };
...
describe('PaymentModal', () => {
    ...
    it('renders reference and notes fields', () => {
        ...
        expect(wrapper.find('input[placeholder="e.g. TXN-12345"]').exists()).toBe(true);
    ...
    it('calls form.post() on submit', async () => {
        ...
        expect(mockPost).toHaveBeenCalledWith('/invoices/42/payments', expect.any(Object));
    });
