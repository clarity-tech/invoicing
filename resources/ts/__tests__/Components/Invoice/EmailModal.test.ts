...
global.fetch = vi.fn().mockResolvedValue({
    ok: true,
    json: () => Promise.resolve({ subject: 'Test Subject', body: '<p>Test body</p>' }),
});
...
function mountComponent(propsOverride = {}) {
    return mount(EmailModal, {
        props: {
            show: true,
            invoice: makeInvoice({ id: 5, invoice_number: 'INV-001', customer_id: 1 }),
            customers: [customer],
            ...propsOverride,
        },
        global: {
...
describe('Invoice/EmailModal', () => {
    ...
    it('calls form.post on Send', async () => {
        ...
        expect(mockPost).toHaveBeenCalledWith('/invoices/5/send-email', expect.objectContaining({ preserveScroll: true }));
    });
