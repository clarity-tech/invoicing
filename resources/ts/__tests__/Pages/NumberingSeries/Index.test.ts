...
type SeriesWithOrg = ReturnType<typeof makeNumberingSeries> & { organization?: ReturnType<typeof makeOrganization> };
...
const defaultProps = () => ({
    ...
    resetFrequencyOptions: { never: 'Never', yearly: 'Yearly', monthly: 'Monthly', financial_year: 'Financial Year' },
});
...
describe('NumberingSeries/Index', () => {
    ...
    it('renders series list', () => {
        const s = makeSeriesWithOrg({ name: 'Default Invoice', prefix: 'INV', format_pattern: '{PREFIX}-{YEAR}-{SEQUENCE:4}', current_number: 5 });
    ...
    it('shows empty state when no series', () => {
        ...
        expect(wrapper.text()).toContain('A default series will be created automatically');
    });

    it('opens create form on button click', async () => {
        const wrapper = mountComponent();
        const createBtn = wrapper.findAll('button').find((b) => b.text() === 'Create New Series' || b.text() === 'Create Custom Series');
    ...
    it('calls router.post for toggle active', async () => {
        ...
        expect(mockRouterPost).toHaveBeenCalledWith('/numbering-series/3/toggle-active', {}, expect.any(Object));
    });
    ...
    it('calls router.post for set default', async () => {
        ...
        const btn = wrapper.findAll('button').find((b) => b.text() === 'Set Default');
        await btn!.trigger('click');

        expect(mockRouterPost).toHaveBeenCalledWith('/numbering-series/7/set-default', {}, expect.any(Object));
    });

    it('does not show Set Default for already default series', () => {
        const s = makeSeriesWithOrg({ is_default: true });
        const wrapper = mountComponent({ series: makePaginatedSeries([s]) });
        const setDefaultBtns = wrapper.findAll('button').filter((b) => b.text() === 'Set Default');
    ...
    it('calls router.delete on confirm delete', async () => {
        ...
        expect(mockRouterDelete).toHaveBeenCalledWith('/numbering-series/9', expect.any(Object));
    });
