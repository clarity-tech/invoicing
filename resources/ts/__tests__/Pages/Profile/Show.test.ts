...
vi.mock('@inertiajs/vue3', () => ({
    ...
    usePage: () => ({
        props: {
            auth: { user: { id: 1, name: 'Test', email: 'test@test.test', profile_photo_url: '' }, currentTeam: { id: 1, name: 'Team', company_name: 'Co', currency: 'INR', personal_team: false } },
...
describe('Profile/Show', () => {
    it('renders all sections when all features enabled', () => {
        ...
        expect(wrapper.find('[data-testid="update-profile-form"]').exists()).toBe(true);
        expect(wrapper.find('[data-testid="update-password-form"]').exists()).toBe(true);
        expect(wrapper.find('[data-testid="two-factor-form"]').exists()).toBe(true);
        expect(wrapper.find('[data-testid="logout-sessions-form"]').exists()).toBe(true);
        expect(wrapper.find('[data-testid="delete-user-form"]').exists()).toBe(true);
    });
    ...
    it('hides profile form when canUpdateProfileInformation is false', () => {
        ...
        expect(wrapper.find('[data-testid="update-profile-form"]').exists()).toBe(false);
    });
    ...
    it('hides 2FA form when canManageTwoFactorAuthentication is false', () => {
        ...
        expect(wrapper.find('[data-testid="two-factor-form"]').exists()).toBe(false);
    });
    ...
    it('hides sessions form when sessionsEnabled is false', () => {
        ...
        expect(wrapper.find('[data-testid="logout-sessions-form"]').exists()).toBe(false);
    });
    ...
    it('hides delete form when hasAccountDeletionFeatures is false', () => {
        ...
        expect(wrapper.find('[data-testid="delete-user-form"]').exists()).toBe(false);
    });
});
