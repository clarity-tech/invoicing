...
import { makeCustomer, makeLocation, makeTaxTemplate, makeNumberingSeries } from '../../helpers';
        ...
        global: {
            stubs: {
                ...
                InvoiceForm: {
                    ...
                    props: ['mode', 'type', 'customers', 'organizationLocations', 'taxTemplates', 'numberingSeries', 'statusOptions', 'defaults'],
                },
            },
        },
    });
...
describe('Invoices/Create', () => {
    it('renders InvoiceForm in create mode', () => {
        ...
        expect(wrapper.find('[data-testid="invoice-form"]').text()).toContain('create');
    ...
    it('passes estimate type to InvoiceForm', () => {
        ...
        expect(wrapper.find('[data-testid="invoice-form"]').text()).toContain('estimate');
    });
    ...
    it('renders with multiple customers', () => {
        const customers = [makeCustomer({ id: 1 }), makeCustomer({ id: 2, name: 'Other Corp' })];
    ...
    it('renders with multiple tax templates', () => {
        const templates = [makeTaxTemplate(), makeTaxTemplate({ id: 2, name: 'VAT 5%' })];
        const wrapper = mountComponent({ taxTemplates: templates });
        expect(wrapper.find('[data-testid="invoice-form"]').exists()).toBe(true);
    });

    it('renders with numbering series', () => {
        const series = [makeNumberingSeries(), makeNumberingSeries({ id: 2, name: 'Estimate Series' })];
