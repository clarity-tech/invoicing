...
const invoiceTemplates = [
    { type: 'invoice_initial', label: 'Initial Invoice', description: 'Sent when sharing an invoice', document_type: 'invoice', is_customized: true },
    { type: 'invoice_reminder', label: 'Invoice Reminder', description: 'Payment reminder', document_type: 'invoice', is_customized: false },
];

const estimateTemplates = [
    { type: 'estimate_initial', label: 'Initial Estimate', description: 'Sent when sharing an estimate', document_type: 'estimate', is_customized: false },
];
...
describe('EmailTemplates/Index', () => {
    ...
    it('shows Edit link for customized templates', () => {
        ...
        const editLink = links.find((a) => a.text() === 'Edit' && a.attributes('href') === '/email-templates/invoice_initial');
    ...
    it('shows Customize link for default templates', () => {
        const wrapper = mountComponent();
        const links = wrapper.findAll('a');
        const customizeLink = links.find((a) => a.text() === 'Customize' && a.attributes('href') === '/email-templates/invoice_reminder');
    ...
    it('renders settings sub-nav with links', () => {
        const wrapper = mountComponent();
        const links = wrapper.findAll('a');
        expect(links.some((a) => a.attributes('href') === '/numbering-series')).toBe(true);
        expect(links.some((a) => a.attributes('href') === '/email-templates')).toBe(true);
    });
});
