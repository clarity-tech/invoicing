    ...
    return mount(EditPage, {
        ...
        global: {
            stubs: {
                ...
                TipTapEditor: {
                    template: '<div data-testid="tiptap-editor"><textarea :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /></div>',
...
describe('EmailTemplates/Edit', () => {
    ...
    it('renders subject input with template value', () => {
        ...
        expect((input.element as HTMLInputElement).value).toBe('Invoice {{invoice_number}} from {{company_name}}');
    });
    ...
    it('shows Save Template button', () => {
        const wrapper = mountComponent();
        const saveBtn = wrapper.findAll('button').find((b) => b.text() === 'Save Template');
    ...
    it('hides Reset to Default button when not customized', () => {
        ...
        const resetBtn = wrapper.findAll('button').find((b) => b.text() === 'Reset to Default');
    ...
    it('calls router.delete on Reset to Default', async () => {
        ...
        expect(mockRouterDelete).toHaveBeenCalledWith('/email-templates/invoice_initial', expect.objectContaining({ preserveScroll: true }));
    });
    ...
    it('hides Restore Default Text when content matches default', () => {
        ...
        const restoreBtn = wrapper.findAll('button').find((b) => b.text() === 'Restore Default Text');
