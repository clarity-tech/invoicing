...
vi.mock('@inertiajs/vue3', () => ({
    ...
    usePage: () => ({
        props: {
            auth: {
                user: { id: 1, name: 'Test', email: 'test@test.test', profile_photo_url: '' },
...
const team = {
    ...
    name: 'Test Team',
    personal_team: false,
    owner: { id: 1, name: 'Owner', email: 'owner@test.test', profile_photo_url: '' },
    users: [],
    team_invitations: [],
};
...
describe('Teams/Show', () => {
    ...
    it('renders UpdateTeamNameForm and TeamMemberManager', () => {
        const wrapper = mount(Show, { props: defaultProps });
        expect(wrapper.find('[data-testid="update-team-name-form"]').exists()).toBe(true);
        expect(wrapper.find('[data-testid="team-member-manager"]').exists()).toBe(true);
    });

    it('shows delete form when canDeleteTeam and not personal team', () => {
        const wrapper = mount(Show, { props: defaultProps });
        expect(wrapper.find('[data-testid="delete-team-form"]').exists()).toBe(true);
    });
    ...
    it('hides delete form when canDeleteTeam is false', () => {
        const wrapper = mount(Show, {
            props: {
                ...defaultProps,
                permissions: { ...defaultProps.permissions, canDeleteTeam: false },
            },
        });
        expect(wrapper.find('[data-testid="delete-team-form"]').exists()).toBe(false);
    });
